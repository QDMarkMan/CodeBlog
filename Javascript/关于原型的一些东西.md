...
流程：**如果我们要查找people中的type， 那么就会按照从实例对象开始的舒徐逐层的向上查找people => people.__proto__(等同于People.prototype) ==>  (people.__proto__).__proto__(等同于Person.prototype) ==> Object.prototype ==> null 一旦中途产生第一个匹配，就停止查找了, 即使上层有同名属性也会返回第一个匹配项,这种情况被称为"属性遮蔽 (`property shadowing`)"。也就是在原型链上查找 如果到了null还是没有，js断言该属性不存在，并给出属性值为 undefined 的结果。**
