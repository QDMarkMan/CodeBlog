...
这些脚本写的时候可能需要一点时间， 但是一旦完成之后就会为团队在效率和质量上有大幅度的提升，让开发人员更见专注与业务和技术。同时时间成本的节约也是不可忽视的，这是我在团队试验之后得出的结论， 以前开发一个模块前期的复制粘贴准备等等可能需要半个小时还要多点， 现在一个模块前期准备加上一个列表页静态开发10分钟搞定。这些都是实在的体现在日常开发中了。另外`Node`环境都安装了， 不用白不用（白嫖😁😁😁）
